---
title: Trochę o JWT i dlaczego nie jest taki świetny, jak się na pozór wydaje
date: 2025-11-20
description: O minusach technologii JWT i dlaczego czasami warto postawić na starą dobrą stanową sesję.
tags: [bezpieczeństwo, szyfrowanie, programowanie]
---

JSON Web Token (w skrócie JWT) stał się w ostatnich latach *złotym młotkiem* web developmentu (zwłaszcza wśród tutoriali
dla początkujących). Traktujemy go jako domyślny standard dla wszystkiego, co wymaga logowania. Obiecuje nam
skalowalność, brak stanu (statelessness) i nowoczesność. Wbrew pozorom, te zalety są jednocześnie największymi wadami
tego rozwiązania.
...
podobnie do sesji. Przeglądarka automatycznie wysyła cookie, serwer weryfikuje token w cookie. Zadajmy zatem pytanie:
*jaka jest właściwie różnica między session id a JWT w ciasteczku poza tym, że w JWT zajmuje dużo więcej bajtów*?
Niewielka, a komplikacji, jak widać, przybywa.
...
urządzeniach mobilnych i IoT. Warto również pamiętać, że standardowe ciasteczka w przeglądarkach mają limit pojemności
wynoszący około 4 KB. Jeśli nasz JWT w wyniku rozrostu ról, uprawnień i danych z IDP (Identity Providera) przekroczy ten
rozmiar, przeglądarka albo go utnie, albo serwer odrzuci nasze zapytanie, rzucając trudnym do zdiagnozowania błędem HTTP
431 *Request Header Fields Too Large*. W przypadku sesji (kilkadziesiąt bajtów) ten problem nigdy nie występuje.
...
właściwego narzędzia do zadania. Myślę, że każdy zaczynający w branży (w tym i ja) na początku wpadał w tę pułapkę, lecz
z czasem należy wyrobić w sobie pragmatyzm, szczególnie dla zagadnień takich jak bezpieczeństwo. Standardowo zapraszam
do dyskusji i dzielenia się własnymi spostrzeżeniami w komentarzach.
