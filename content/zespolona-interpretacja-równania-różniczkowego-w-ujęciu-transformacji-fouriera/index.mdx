...
description: |
  ...
  w dziedzinie czasu to proste mnożenie w analizie Fouriera. Symulacje z wykorzystaniem generatora opartego o ESP32.
...
przeze mnie [pracy magisterskiej](https://github.com/milosz08/master-thesis), poświęconej nadpróbkowanej transformacji
falkowej.
...
Otrzymaliśmy wynik matematycznie poprawny, ale jest on w cosinusie, a prąd chcemy mierzyć w sinusie. Żeby porównać te
dwa sygnały (i tym samym zobaczyć przesunięcie fazy) trzeba zmienić cosinus z powrotem na sinus, z użyciem tożsamości
trygonometrycznej:
...
Wróćmy natomiast jeszcze do wzoru $u(t)=\omega L \sin\left(\omega t+\frac{\pi}{2}\right)$. Na pierwszy rzut oka wynik
...
Musimy jakość znaleźć wartość $h(t)$ a więc odpowiedź impulsową cewki z założeniem, że cewka różniczkuje. Tutaj właśnie
...
### Eksperyment 2: Splot w czasie to odcięcie harmonicznych w widmie
...
Poniższy kod na kanale DAC1 generuje idealnie czystą sinusoide 50Hz. Na kanale DAC2 wykonywany jest matematyczny *splot*
(sumowanie) w dziedzinie czasu: do podstawowej sinusoidy 50Hz dodawana jest trzecia harmoniczna ($3f$) o mniejszej
amplitudzie.
...
void loop() {
  ...
  // kanał 2: splot sygnału o mniejszej amplitudzie
...
Na poniższym rysunku widać w dziedzinie czasu czystą sinusoide 50Hz (żółty przebieg) oraz przebieg odkształcony (kolor
niebieski). Operacja FFT wykonywana była dla amplitudy w skali liniowej (w celu czystego obrazu prążków) wraz z oknem
Hanninga (przy oknie prostokątnym obserwowany był przeciek widma). Wykonując operację FFT na żółtym przebiegu widać
jeden dominujący prążek w granicach 50Hz. Natomiast FFT na przebiegu odkształconym wykryte zostały dwa prążki: jeden
podstawowy (około 50Hz) oraz drugi jako trzecia harmoniczna (około 156Hz, trzykrotność częstotliwości podstawowej).
...
czyli 2.5. Pomiary oscyloskopowe pokazują 84.31 mVrms dla składowej podstawowej oraz 32.43 mVrms dla trzeciej
harmonicznej. Stosunek ten wynosi $\frac{84.31}{32.43}\approx 2.6$, co potwierdza, że DAC w ESP32 zachowuje liniowość.
...
dla modulacji AM). Środkowy prążek około 206 Hz to fala nośna, a symetrycznie po jej bokach widoczne są dwa nowe prążki
(wstęgi boczne) oddalone o częstotliwość modulująca, około 20 Hz.
...
nośnej: około 48 mVrms, amplitudy wstęg bocznych: około 24 mVrms).
...
* W teorii obwodów jednostka urojona $j$ nie jest jedynie abstrakcją matematyczną, lecz operatorem różniczkowania w
  ...
  sygnałów objawia się jako fizyczne przesunięcie fazowe o $90^\circ$.
